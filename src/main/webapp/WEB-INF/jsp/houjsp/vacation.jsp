  ...
  <script type="text/javascript">
        ...
        $(function(){
      	  ...
      	  $.ajax({
        		type : 'post',
        		dataType : 'json',
        		url : '<%=basePath%>manager/findvactiondayAll',
        		success : function(data) {	
        			var $tbody = $("#appl");
        			$tbody.empty();  
        			for (var j = 0; j < data.length; j++) { 
    	            	
    	            	var unixTimestamp = new Date(data[j].attdVactionBegintime) ;
    	            	var attdVactionBegintime = unixTimestamp.toLocaleString();
    	            	
    	            	var unixTimestamp2 = new Date(data[j].attdVactionEndtime) ;
    	            	var attdVactionEndtime = unixTimestamp2.toLocaleString();
    	            	var type = "是";
    	            	if (attdVactionEndtime=="") {
    	            		type = "否"
    					}
    	            	
    	            	var k = j+1;
    	            	var table="<tr class='gradeX'><td>"+k+"</td><td><a href='javascript:void(0);' title='员工历史考勤数据' style='color:#0080FF'  id='findmanayuser'  onclick=toremind("+data[j].usernumber+")>"+data[j].usernumber+"</a></td><td>"+data[j].username+"</td><td>"+attdVactionBegintime+"</td><td>"+attdVactionEndtime+"</td><td>"+type+"</td></tr>";
...
<script type="text/javascript">
	function toremind(userNumber) {
		...
		$.ajax({
	 		type : 'post',
	 		dataType : 'json',
	 		url : '<%=basePath%>manager/findUserVactiondayAll?usernumber=' + usernumber,
	 		success : function(data) {	
    			var $tbody = $("#appl2");
    			$tbody.empty();  
					for (var j = 0; j < data.length; j++) { 
	            	...
	            	var k = j+1;
	            	var table="<tr class='gradeX'><td>"+k+"</td><td>"+data[j].username+"</td><td>"+data[j].attdVactionRemark+"</td><td>"+attdVactionBegintime+"</td><td>"+attdVactionEndtime+"</td><td>"+type+"</td></tr>";
	...
	function tofind() {
		...
		$.ajax({
	  		type : 'post',
	  		dataType : 'json',
	  		url : '<%=basePath%>manager/findUserVactionday?usernumber=' + text,
	  		success : function(data) {	
	  			var $tbody = $("#appl");
	  			$tbody.empty();  
	  		    for (var j = 0; j < data.length; j++) { 
