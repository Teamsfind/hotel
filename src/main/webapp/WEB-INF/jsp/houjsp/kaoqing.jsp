  ...
  <script type="text/javascript">
        ...
        $(function(){
      	  ...
      	  $.ajax({
        		type : 'post',
        		dataType : 'json',
        		url : '<%=basePath%>manager/findattdayAll',
        		success : function(data) {	
        			var $tbody = $("#appl");
        			$tbody.empty();  
        	            for (var j = 0; j < data.length; j++) { 
        	            	
        	            	var unixTimestamp = new Date(data[j].attdBegintime) ;
        	            	var attdBegintime = unixTimestamp.toLocaleString();
        	            	
        	            	var unixTimestamp2 = new Date(data[j].attdEndtime) ;
        	            	var attdEndtime = unixTimestamp2.toLocaleString();
        	            	var type = "正常";
        	            	if (unixTimestamp.getHours()>=9) {
        	            		type = "迟到"
							}
        	            	if(unixTimestamp2.getHours<=5){
								type = "早退"
							}
...
<script type="text/javascript">
	function toremind(userNumber) {
		...
		$.ajax({
	 		type : 'post',
	 		dataType : 'json',
	 		url : '<%=basePath%>manager/findUserAttdayAll?usernumber=' + usernumber,
	 		success : function(data) {	
    			var $tbody = $("#appl2");
    			$tbody.empty();  
    	            for (var j = 0; j < data.length; j++) { 
    	            	...
    	            	var k = j+1;
    	            	var table="<tr class='gradeX'><td>"+k+"</td><td>"+data[j].usernumber+"</td><td>"+data[j].username+"</td><td>"+attdBegintime+"</td><td>"+attdEndtime+"</td><td>"+type+"</td></tr>";
	...
	function tofind() {
		...
		$.ajax({
	  		type : 'post',
	  		dataType : 'json',
	  		url : '<%=basePath%>manager/findUserAttday?usernumber=' + text,
	  		success : function(data) {	
	  			var $tbody = $("#appl");
	  			$tbody.empty();  
	  		    for (var j = 0; j < data.length; j++) { 
